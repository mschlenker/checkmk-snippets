...
#define LORA_AMPEL 1 //1 = Version mit LoRa
...
#define LORA_BANDWIDTH     62.5E3 //Bandwidth in Hz
...
void lora_service(void)
{
  ...
  if(features & (FEATURE_LPS22HB|FEATURE_BMP280))
  {
    sprintf(buf,
        ...
        " \"f\": %s " \
        "}\r\n",
        LORA_NODENAME, co2_value, temp_value, humi_value, pres_value, temp2_value, light_value, VERSION
    );
  }
  else
  {
    sprintf(buf,
      ...
      " \"f\": %s " \
      "}\r\n",
      LORA_NODENAME, co2_value, temp_value, humi_value, light_value, VERSION
    );
  }
  LoRa.beginPacket();
  LoRa.print(buf);
...
void loop()
{
  ...
  static unsigned long t_switch=0, t_ampel=0, t_lora, t_light=~((LICHT_INTERVALL*1000UL*60UL)-60000UL); //Lichtsensor nach 60s pruefen
  ...
  #if LORA_AMPEL
    if((millis()-t_lora) > LORA_INTERVALL)
    {
      t_lora = millis(); //Zeit speichern
      //LoRa-Daten senden
      lora_service();
    }
  #endif
