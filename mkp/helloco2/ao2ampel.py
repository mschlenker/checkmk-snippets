...
def check_co2ampel(params, section):
    for line in section:
        ...
        if (line[0] == "co2"):  
            yield Metric(name="co2", value=int(line[1]), boundaries=(0, 10000))
        ...
        if float(line[1]) > params["co2levels"][1]:
            yield Result(state=State.CRIT, summary="Open windows immediately!") 
            return
        elif float(line[1]) > params["co2levels"][0]:
            yield Result(state=State.WARN, summary="Try to let in clean air!")
            return
        yield Result(state=State.OK, summary="The air is just fine.")
...
register.check_plugin(
    ...
    service_name = "Watterott CO2 Ampel",
    # Refer to the discovery function above.
    discovery_function = discover_co2ampel,
    # Refer to the check function above.
    check_function = check_co2ampel,
    # Refer to the ruleset defined in:
    # ~/local/share/check_mk/web/plugins/wato/helloworld_parameters.py
    check_ruleset_name = "co2ampel",
    # check_default_parameters = {}
    # if defaults have to be defined here, use:
    check_default_parameters = { "levels" : (800, 1100) }
)
